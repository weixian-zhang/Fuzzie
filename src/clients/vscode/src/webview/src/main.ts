      ...
      override: [function myCompletions(context: CompletionContext){
        ...
        if (word != undefined && word.from == word.to && !context.explicit)
          ...
          return {
            ...
            options: [
              {label: "bool", type: "keyword", apply: "{{ bool }}", info: "primitive wordlist"},
              {label: "digit", type: "keyword", apply: "{{ digit }}", info: "primitive wordlist"},
              {label: "mutate", type: "keyword", apply: "{{ 'this custom input will be mutated' | mutate }}", info: "mutate custom input"},
              {label: "myfile", type: "keyword", apply: "{{ 'this is a file content, and supports primitive wordlist like {{ string }}' | myfile('optional-filename.log') }}", info: "mutate custom input"},
              {label: "char", type: "keyword", apply: "{{ char }}", info: "primitive wordlist"},
              {label: "image", type: "keyword", apply: "{{ image('image.png') }}", info:"Images mix with other file types encode with 'latin1'"},
              {label: "pdf", type: "keyword", apply: "{{ pdf('report.pdf') }}", info:"PDF mix with payload from danielmiessler seclist"},
              {label: "file", type: "keyword", apply: "{{ file('log.txt') }}", info:"danielmiessler's seclist payload encode with 'latin1'"},
              {label: "datetime", type: "keyword", apply: "{{ datetime }}", info:"primitive wordlist - date & time"},
              {label: "date", type: "keyword", apply: "{{ date }}", info:"primitive wordlist - date only"},
              {label: "time", type: "keyword", apply: "{{ time }}", info:"primitive wordlist - time only"},
              {label: "username", type: "keyword", apply: "{{ username }}", info:"primitive wordlist - usernames from danielmiessler seclist"},
              {label: "password", type: "keyword", apply: "{{ password }}", info:"primitive wordlist - password from danielmiessler seclist"},
              {label: "filename", type: "keyword", apply: "{{ filename }}", info:"primitive wordlist - file names"},
              ...
              {label: "httppath", type: "keyword", apply: "{{ httppath }}", info:"http path discovery, use to discover directories and files. From danielmiessler/seclist and https://wordlists.assetnote.io"},
